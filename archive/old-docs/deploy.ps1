...
if ($gitStatus) {
    ...
    if ($commitChoice -eq "y" -or $commitChoice -eq "Y") {
        ...
        Write-Host "WARNING: Proceeding with existing committed changes only" -ForegroundColor Yellow
    }
}
...
if ($repoUrl -match "github\.com[:/](.+?)(?:\.git)?$") {
    ...
    Write-Host "WARNING: Could not determine GitHub repository URL" -ForegroundColor Yellow
